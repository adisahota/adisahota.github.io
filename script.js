...
window.onload = function() { // When the window loads, run the following code
  ...
  function changeBackgroundColor(newColor) { //change background color
    let body = document.getElementById("body");
    body.style.backgroundColor = newColor;
  }

  function changeHeadColor(newColor) { //change header colors
  ...
  function changeTextColor(newColor) { //change all text colors
    let elements = document.querySelectorAll("body *:not(.header):not(a):not(footer p):not(button):not(#index *)"); // Select all elements except the one with id "header"
...
var slideIndex = 1; // Define the initial slide index value
showDivs(slideIndex);

//a function to change the current slide by a given amount
function plusDivs(n) {
  showDivs(slideIndex += n);
}


//a function to show a slide with a given index and hide all others
function showDivs(n) {
